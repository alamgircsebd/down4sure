<?php
/*
Plugin Name: Utillz Login
Plugin URI: n/a
Description:  Social Login
Version: 2.0.0
Author: Utillz
Author URI: utillz.com
Text Domain: Utillz Login
Domain Path: /languages
*/
defined('ABSPATH') or die("Cheating........Uh!!");
define('THE_UTILLZ_LOGIN_VERSION', '2.0.0');
...
function the_champ_connect(){
	...
	if(isset($_GET['SuperSocializerAuth'])){
		if(sanitize_text_field($_GET['SuperSocializerAuth']) == 'Vkontakte'){
		    ...
		    if(isset($theChampLoginOptions['providers']) && (in_array('google', $theChampLoginOptions['providers']) || in_array('youtube', $theChampLoginOptions['providers'])) && isset($google_key) && $google_key != '' && isset($google_secret) && $google_secret != ''){
		        ...
		        wp_redirect("https://accounts.google.com/o/oauth2/auth?client_id=" . $google_key . "&scope=https://www.googleapis.com/auth/userinfo.email%20https://www.googleapis.com/auth/userinfo.profile&state=". $googleLoginState ."&response_type=code&prompt=select_account&redirect_uri=" . home_url());
		        die;
		    }
		}elseif(sanitize_text_field($_GET['SuperSocializerAuth']) == 'Youtube'){
		    if(isset($theChampLoginOptions['providers']) && in_array('youtube', $theChampLoginOptions['providers']) && isset($theChampLoginOptions['youtube_key']) && $theChampLoginOptions['youtube_key'] != '' && isset($google_key) && $google_key != '' && isset($google_secret) && $google_secret != ''){
	            ...
	            wp_redirect("https://accounts.google.com/o/oauth2/auth?client_id=" . $google_key . "&scope=https://www.googleapis.com/auth/userinfo.email%20https://www.googleapis.com/auth/youtube.readonly&state=". $youtubeLoginState ."&response_type=code&prompt=select_account&redirect_uri=" . home_url());
		        die;
		    }
		    ...
		    if(isset($theChampLoginOptions['providers']) && in_array('facebook', $theChampLoginOptions['providers']) && isset($fb_app_id) && $fb_app_id != '' && isset($fb_secret) && $fb_secret != ''){
		        if(!isset($_GET['code'])){
		            ...
		            wp_redirect("https://www.facebook.com/v18.0/dialog/oauth?scope=email&client_id=" . $fb_app_id . "&state=" . $facebookLoginState . "&redirect_uri=" . home_url() . "/?SuperSocializerAuth=Facebook");
		            die;
...
function the_champ_frontend_scripts(){
	...
	<?php
	if(!$combinedScript){
		wp_enqueue_script('the_champ_ss_general_scripts', plugins_url('js/front/social_login/general.js', __FILE__), false, THE_UTILLZ_LOGIN_VERSION, $inFooter);
	}
	...
	if(the_champ_social_login_enabled() && !is_user_logged_in()){
		...
		<?php
		if(!$combinedScript){
			wp_enqueue_script('the_champ_sl_common', plugins_url('js/front/social_login/common.js', __FILE__), array('jquery'), THE_UTILLZ_LOGIN_VERSION, $inFooter);
		}
	...
	if(the_champ_facebook_plugin_enabled()){
		...
		if(!$combinedScript){
			wp_enqueue_script('the_champ_fb_sdk', plugins_url('js/front/facebook/sdk.js', __FILE__), false, THE_UTILLZ_LOGIN_VERSION, $inFooter);
		}
	}
	...
	if(the_champ_social_commenting_enabled()){
		...
		if($post){
			...
			if(isset($theChampFacebookOptions['enable_'.$post->post_type]) && !(isset($postMeta) && isset($postMeta['fb_comments']) && $postMeta['fb_comments'] == 1)){
				...
				<?php
				if(!$combinedScript){
					wp_enqueue_script('the_champ_fb_commenting', plugins_url('js/front/facebook/commenting.js', __FILE__), false, THE_UTILLZ_LOGIN_VERSION, $inFooter);
				}
			}
		}
	}
	...
	if($combinedScript){
		wp_enqueue_script('the_champ_combined_script', plugins_url('js/front/combined.js', __FILE__), array('jquery'), THE_UTILLZ_LOGIN_VERSION, $inFooter);
	}
}
...
function the_champ_frontend_styles(){
	...
	<?php
	wp_enqueue_style('the_champ_frontend_css', plugins_url('css/front.css', __FILE__ ), false, THE_UTILLZ_LOGIN_VERSION);
...
function the_champ_create_admin_menu(){
	$page = add_menu_page('Utillz Login by Utillz', 'Social Login', 'manage_options', 'utillz-ul-general-options', 'the_champ_general_options_page');
	// general options page
	$generalOptionsPage = add_submenu_page('utillz-ul-general-options', __( "Social Login - General Options", 'utillz-login'), __( "General Options", 'utillz-login'), 'manage_options', 'utillz-ul-general-options', 'the_champ_general_options_page');
	
	$loginPage = add_submenu_page('utillz-ul-general-options', 'Utillz Login - Social Login', 'Social Login', 'manage_options', 'utillz-social-login', 'the_champ_social_login_page');
	// social sharing page
...
function the_champ_save_default_options(){
	...
	add_option('the_champ_ss_version', THE_UTILLZ_LOGIN_VERSION);
}
...
function the_champ_update_db_check(){
	...
	if($currentVersion && $currentVersion != THE_UTILLZ_LOGIN_VERSION){
		...
		update_option('the_champ_ss_version', THE_UTILLZ_LOGIN_VERSION);
	}
}
